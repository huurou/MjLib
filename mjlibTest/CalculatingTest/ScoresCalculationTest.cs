...
namespace mjlibTest.CalculatingTest
{
    ...
    public class ScoresCalculationTest
    {
        ...
        public void CalculateScoresAndRonTest()
        {
            ...
            var result = CalculateScore(han: 1, fu: 30, config: config);
            AreEqual(1000, result.Main);

            result = CalculateScore(han: 1, fu: 110, config: config);
            AreEqual(3600, result.Main);

            result = CalculateScore(han: 2, fu: 30, config: config);
            AreEqual(2000, result.Main);

            result = CalculateScore(han: 3, fu: 30, config: config);
            AreEqual(3900, result.Main);

            result = CalculateScore(han: 4, fu: 30, config: config);
            AreEqual(7700, result.Main);

            result = CalculateScore(han: 4, fu: 40, config: config);
            AreEqual(8000, result.Main);

            result = CalculateScore(han: 5, fu: 0, config: config);
            AreEqual(8000, result.Main);

            result = CalculateScore(han: 6, fu: 0, config: config);
            AreEqual(12000, result.Main);

            result = CalculateScore(han: 8, fu: 0, config: config);
            AreEqual(16000, result.Main);

            result = CalculateScore(han: 11, fu: 0, config: config);
            AreEqual(24000, result.Main);

            result = CalculateScore(han: 13, fu: 0, config: config);
            AreEqual(32000, result.Main);

            result = CalculateScore(han: 26, fu: 0, config: config);
            AreEqual(64000, result.Main);

            result = CalculateScore(han: 39, fu: 0, config: config);
            AreEqual(96000, result.Main);

            result = CalculateScore(han: 52, fu: 0, config: config);
            AreEqual(128000, result.Main);

            result = CalculateScore(han: 65, fu: 0, config: config);
            AreEqual(160000, result.Main);

            result = CalculateScore(han: 78, fu: 0, config: config);
        ...
        public void CalculateScoresAndTsumo()
        {
            ...
            result = CalculateScore(han: 3, fu: 60, config: config);
            AreEqual(3900, result.Main);
        ...
        public void KiriageManganTest()
        {
            ...
            var result = CalculateScore(han: 4, fu: 30, config: config);
            AreEqual(8000, result.Main);
