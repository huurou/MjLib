...
internal static class HandDevider
{
    ...
    public static List<TileKindListList> Devide(TileKindList hand)
    {
        ...
        {
            var copiedArray = new TileCountArray(countArray_.ToArray());
            // 雀頭候補を外す
        ...
        var uniqueHands = new List<TileKindListList>();
        ...
        {
            hands.Add(new(toitsuKinds.Select(x => new TileKindList(Enumerable.Repeat(x, 2)))));
        }
        hands.Sort((x, y) =>
        {
    ...
    private static List<TileKindListList> FindValidCombinations(TileCountArray countArray, TileKind startKind, bool handNotCompleted = false)
    {
        // countArrayから指定のスートのTileKindListを作成する
        var kindList = new TileKindList(Enumerable.Range(startKind.Id, 9).SelectMany(x => Enumerable.Repeat(new TileKind(x), countArray[x])));
        if (kindList.Count == 0) return new();
        // TileKindListのnP3全順列を列挙する
        // [1,2,3,4,4]=>[[1,2,3],[1,2,4],[1,3,2],[1,3,4],[1,4,2],[1,4,3],[1,4,4],...]
        var perms = GetPermutations(kindList, 3).Select(x => new TileKindList(x));
        var validPerms = new TileKindListList(perms.Where(x => x.IsShuntsu || x.IsKoutsu));
        if (validPerms.Count == 0) return new();
        ...
        if (handNotCompleted) return new() { validPerms };
        // 可能な組み合わせを探す
        var combinationsResults = new List<TileKindListList>();
        foreach (var perm in GetPermutations(Enumerable.Range(0, validPerms.Count), neededPermsCount))
        {
            ...
            var results = new TileKindListList(perm.Select(x => validPerms[x]).OrderByDescending(x => x[0]));
