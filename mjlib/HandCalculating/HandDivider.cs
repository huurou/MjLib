...
namespace mjlib.HandCalculating
{
    ...
    {
        public static IList<List<TileKinds>> DivideHand(Tiles34 tiles34, IEnumerable<Meld>? melds = null)
        {
            if (melds is null)
            {
                melds = new List<Meld>();
            ...
            var openTileIndices = melds.Any()
                ? melds.Select(x => x.TileKinds).Aggregate((x, y) => new TileKinds(Enumerable.Concat(x, y)))
                : new TileKinds();
        ...
        private static bool Contains_<T>(this IEnumerable<T> source, T item) where T : IEnumerable<TileKinds> => source.Any(x =>
                                                                                                                           {
                                                                                                                               var y = x.ToList();
                                                                                                                               var z = item.ToList();
